...
| Chapter review (edit/save) + Chapter QA (factcheck+editor track-changes, production; suggestions ordered by position in text; qaPane/qaPaneCompare are contenteditable with locked suggestion spans (`tabindex="-1"`, excluded from Tab order), untouched prose stays directly editable) | `app/controllers/chapter_review_controller.rb`, `app/views/chapter_review/`, `app/javascript/controllers/chapter_review_controller.ts`, `app/services/pipeline/ruby/chapter_qa.rb`, `PromptBuilder.build_chapter_qa_factcheck_system_prompt`/`.build_chapter_qa_editor_system_prompt` | `docs/DECISIONS.md` (chapter_qa entry, 2026-08-08 suggestion-ordering/no-forced-scroll/direct-editing/tab-order/bible-lookup entries) |
| bible-lookup editor function (select text + Tab → search/add to bible) — mounted on every editable text field, not just one page: `chapter_review` (textareas + the contenteditable QA pane) and `chapters#show`'s single-chapter editor (`chapter_viewer_controller.ts`, plain textareas only) | `app/javascript/controllers/bible_lookup_controller.ts` | `docs/DECISIONS.md` (2026-08-08 bible-lookup entry) |
