...
# superseded, kept for history) single-call structured prompt, and Call 1 of
# the committed 2-call pipeline (docs/DECISIONS.md's 2026-07-30 entry) over
# the same chapters, writing all outputs to disk for a human to compare. Not
# wired into translate_batch.rb, PipelineJob, or any TranslationJob: the
# point is to judge quality/cost/JSON-reliability — and, for Call 1
# specifically, segmentation exhaustiveness and passage_id stability —
  ...
  module Ruby
    class TranslationEval
      ...
      def call
        ...
        @chapter_numbers.map { |num| run_chapter(num, single_pass_prompt, structured_prompt, call1_prompt, mcp_config) }
      end
      ...
      def run_chapter(num, single_pass_prompt, structured_prompt, call1_prompt, mcp_config)
      ...
      # Returns [call_error, valid_json, segmentation_error]. Segmentation is
      # only checked once the response is valid JSON — it's the thing this
      ...
      def run_call1(chapter_dir, system_prompt, korean_text, mcp_config)
        ...
        [ nil, true, validate_segmentation(korean_text, parsed) ]
      end
