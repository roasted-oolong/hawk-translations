...
**Status:** Design approved 2026-08-09, implementation plan approved 2026-08-09. Groups A and B shipped 2026-08-10 (`Pipeline::BibleEntryDocWriter`/`BibleDocSynced`, `bible_entry_proposals` staging table, `Pipeline::BibleEntryMatcher`/`BibleEntryProposalIngester`, `PrereadRunner` wired to the ingester, rebuilt review controllers + slideshow — see `docs/DECISIONS.md`'s 2026-08-10 entries). Group C (translate lock) and Group D (backfill + `BibleMarkdownParser`/`PrereadBibleWriter` deletion) not started.
...
- **Group C**: `TranslationJob#chapter_bible_proposals_resolved` validation (mirrors existing `voice_calibration_chapter_reviewed`).
- **Group D**: backfill via one-off Rake task (not a migration — needs runtime file reads + human judgment on the named stale mismatches) → delete `BibleMarkdownParser`/`PrereadBibleWriter` + specs, leaving `BibleFileEditor` and the `preread_dismissed_keys` column intact.
...
**Status as of 2026-08-10: Groups A and B shipped and committed** on `translation-quality-pipeline` (10 commits: B1 table/model, a same-day fix for a `skip!` dismissed-key vocabulary bug B1 shipped with, B2 `Novel#append_preread_dismissed_key!` extraction, B3 `BibleEntryMatcher` extraction, B4 `BibleEntryProposalIngester`, B5 `PrereadRunner` wiring, B6 controllers/routes, B7 slideshow view/JS). Full spec suite green throughout, diffed against a stashed pre-change baseline after B1–B4 (identical 55 pre-existing failures both sides) and re-confirmed after B5/B6/B7 (19-failure subset of the same list when run without `spec/system/`) — zero regressions at every step. Not pushed to origin as of writing. **Group C not started** — next session picks up there: `TranslationJob#chapter_bible_proposals_resolved`, mirroring the existing `voice_calibration_chapter_reviewed` guard.
