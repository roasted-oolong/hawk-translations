...
RSpec.describe Pipeline::Ruby::BibleBuild do
  it "calls PrereadRunner with the all-korean-chapters discovery predicate and batch size 5" do
    job = build_stubbed(:translation_job, :bible_build)
    captured = nil
    ...
    expect(captured[:batch_size]).to eq(5)
    ...
    Dir.mktmpdir do |dir|
      ...
      expect(captured[:discovery].call(dir)).to eq([ 1, 2 ])
    end
  end
end
